...
planner can stand in for a learned policy, and \emph{where} it cannot: does
its success track the dynamical complexity of the environment? Robot-policy
success rates are reported per paradigm --- pretrained imitation,
fine-tuning, classical control, world-model planning --- on incompatible
protocols, so the paradigms cannot be put on one axis. \emph{Building a
ruler that scores them together is the load-bearing contribution of this
paper.} We introduce \emph{embodimetry}, a
\emph{Capability-Ladder Audit}, and stand behind three reproducible
contributions. \emph{First, a single cross-paradigm evaluation contract}:
pretrained, fine-tuned, classical-control, and world-model-planner policies
are all scored as the \emph{same} \texttt{PolicyCallable} under one seed /
Wilson-and-bootstrap-CI / minimum-detectable-effect (MDE) ruler on shared
LeRobot~\cite{cadene2024lerobot} tasks, with floor baselines and honest
negatives (a SmolVLA-LoRA collapse, a classical controller at $0.012$) kept
on the board --- no neighboring effort puts paradigms on one ruler with the
negatives retained. \emph{Second, a self-auditing methodology}: the
instrument caught and corrected its \emph{own} headline overclaim --- a
normalization bug in our harness pinned ACT on Aloha-transfer-cube at
...
over-claimed top rung stays labeled to its evidence; an eval suite must be
audited as carefully as the policies it scores. \emph{Third, a
strengthened-but-qualified L3 reading the contract was built to test}:
under matched receding-horizon MPC, zero-training world-model latent-MPC
planning solves \emph{navigation} but not \emph{contact}, replicated across
two independent world-model families (DINO-WM,
\texttt{facebookresearch/jepa-wms}) --- Wall navigation JEPA-WM $4/6
\approx$ DINO-WM $3/6$, PushT contact JEPA-WM $0/6 \approx$ DINO-WM $0/6$.
An earlier one-shot $0/6$ JEPA-WM Wall reading was a
\emph{planning-protocol artifact} (full-horizon vs.\ receding-horizon),
corrected by a protocol control; the contact endpoint stays ${\sim}0$
\emph{even with} receding-horizon and across both families, so the
nav$>$contact gap is neither a protocol artifact nor
checkpoint-specific. We report this as a strengthened result, not a
cross-env law: it is a \emph{two-endpoint} contrast at $N{=}6$ episodes per
cell with one environment per endpoint, so the gradient \emph{middle} is
unmeasured (a MetaWorld anchor is future work); the L3 cells run as a
cross-repo artifact (\texttt{lerobot-wm-research}) on the same eval
contract. \textbf{L0} (pretrained Diffusion Policy on PushT) scores
$0.816$~[0.739, 0.874]. \textbf{L1} (continued fine-tuning of an
already-converged ACT on Aloha-transfer-cube) moves $0.824$~[0.772, 0.866]
to $0.864$~[0.816, 0.901] --- a $+0.040$ shift whose CIs \emph{overlap}
(Cohen's $h{=}0.11$, below the $N{=}250$ MDE), reported as within noise; an
attempted SmolVLA LoRA fine-tune collapses ($0.252\to0/50$ on a seed-0
probe). \textbf{L2}
(a competent scripted PushT controller) clears the strict bar on only
$0.012$~[0.004, 0.035] of episodes despite mean rollout coverage near
$0.50$. The contributions we stand behind are the contract, the
self-audit, and the L3 two-endpoint nav-vs-contact replication; the full
...
planning solves \emph{navigation} but not \emph{contact}, replicated across
two independent world-model families, a \emph{two-endpoint} contrast
(\S\ref{sec:l3}) rather than a measured curve or cross-env law. We release
embodimetry across three surfaces: open data (a public Hugging Face
Hub~\cite{huggingfacehub2024} dataset of every per-episode outcome plus an
MP4 of every rollout); open eval (a pinned, scriptable pipeline that
re-derives any parquet row in $<1$\,h on commodity hardware); and open
analysis (the notebook \texttt{notebooks/01-write-finding.ipynb} that
produces every table and figure here). The v1 leaderboard is four learned
...
receding-horizon MPC, a zero-training world-model planner solves
navigation (Wall, JEPA-WM $4/6\approx$ DINO-WM $3/6$) but not contact
(PushT, $0/6\approx0/6$), at ${\sim}1000\times$ the per-step compute of the
learned diffusion policy that scores $0.816$ on the same env; an earlier
apparent $0/6$ Wall failure was a planning-protocol artifact, corrected by
a protocol control, and the split replicates across two independent
world-model families, so it is neither a protocol artifact nor
checkpoint-specific (\S\ref{sec:l3}). The contract is what made this
...
defaults supplied only the step caps --- and v1.1 reruns affected cells.
...
provenance accordingly rather than fabricating an in-tree artifact.
...
every step), a zero-training world-model latent-MPC planner solves
...
(JEPA-WM) $\approx 3/6$ (DINO-WM); on \textbf{PushT}, quasi-static contact
manipulation, it scores $0/6\approx0/6$, moving the block but pushing it
away from the goal, at ${\sim}1000\times$ the per-step compute of the
learned diffusion policy that scores $0.816$ on the same env. Two confounds
...
endpoint stays ${\sim}0$ \emph{even with} receding-horizon and across both
families, so the nav$>$contact gap is neither a protocol artifact nor
checkpoint-specific. One confound remains uncontrolled: the two cells still
...
uniform-random rate. Floor baselines sit at the far left and the SmolVLA
per-suite gradient (\texttt{goal}~$0.928$ down to \texttt{libero\_10}~$0.252$)
spreads across the axis. \texttt{xvla\_libero} excluded (deferred). Source:
...
points carry a ``(single-task)'' tag: the v1 sweep ran \texttt{task\_id=0}
only, vs.\ the paper's 10-task suite averages, so those gaps are not
...
For context, SmolVLA's per-suite rates under our protocol
(\texttt{task\_id=0}, env-default caps) span a sharp gradient (0.928
\texttt{goal} down to 0.252 \texttt{libero\_10}); these are single-task
envelopes, not suite
...
patch rewrites a parquet row. Alongside label-free failure signals
(episode-length and cap-hit distributions, plus every failed-episode MP4)
we hand-label a small set of failed rollouts into the canonical six-mode
taxonomy (Figure~\ref{fig:failure-taxonomy}); a parquet-joined,
randomly-sampled per-policy taxonomy is deferred to v1.1.
...
\begin{figure}[t]
  ...
  shown as empty bars rather than dropped. Curated, non-random set, not a
  per-policy distribution; XVLA excluded (wiring bug, not a policy mode).
...
code-enforces that the rung above keeps its caveats. The supporting spine
is a set of honest negatives that earn the audit its credibility: every
L0--L2 row is bit-replayable and Wilson/bootstrap CIs agree within one
half-width; fine-tuning a near-ceiling policy moves it within noise
($+0.040$, overlapping CIs, $h{=}0.11$) while an attempted SmolVLA LoRA
...
result at $N{=}6$ per cell with one environment per endpoint --- a
...
trained on --- for \emph{both} world-model families, which is why it reads
as a property of the dynamics, not of one checkpoint. PushT's dynamics are
contact-rich and stiff, so small latent errors compound through contact and
the planner pushes the block confidently the wrong way --- again for both
families, so the contact failure is robust rather than a single-checkpoint
artifact. So ``do we need a learned policy?'' already looks, at the two
...
(${\sim}1000\times$ per step) reported alongside.
...
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}\setlength{\topsep}{0pt}\setlength{\partopsep}{0pt}
    ...
    (coarse per-cell rates) and there is one environment per endpoint, so
...
Close the audit caveats (cap=600 LIBERO reruns); onboard the $\pi_0$
