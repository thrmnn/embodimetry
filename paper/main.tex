...
\caption{Failure taxonomy: hand-labeled failed rollouts assigned to the six
canonical modes (first-fit in chronological order, one draw per seed). Counts
are sourced from the labeled rows in
\texttt{docs/FAILURE\_TAXONOMY.md}; v1 is single-labeler and a per-rollout
\texttt{failure\_mode} column is a pending schema bump. wrong-object is
highlighted as the mode the \texttt{libero\_object} cluster localises to.}
