...
fine-tuning a near-ceiling policy shifts the number by an amount
\emph{inconclusive at $N{=}250$} (L1, ACT); an attempted SmolVLA LoRA
fine-tune collapses rather
...
\emph{episodes} not seeds); the two agree within 0.6\,pp on the
leaderboard cells, including the down-scoped $N{=}125$ cell. For
...
(Table~\ref{tab:act-ablation}): the recovery is attributable to the
...
McNemar $p{=}0.23$; Table~\ref{tab:act-ablation}), so temporal ensembling
...
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}\setlength{\topsep}{0pt}\setlength{\partopsep}{0pt}
    ...
    this cell; we can only show the
