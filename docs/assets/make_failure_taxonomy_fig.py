...
shipped rollout artifacts) and writes
``fig-failure-taxonomy.{svg,png}`` (docs/Space style) keyed on the
*observed* modes that actually appear in the artifacts.

The paper's taxonomy figure is no longer rendered here: the v1 curated
pass was superseded by the v1.1 parquet-joined pass
(``failure-taxonomy-labels-v11.csv``), rendered by
``embodimetry.figures.failure_taxonomy_v11`` via ``make paper-figures``.
